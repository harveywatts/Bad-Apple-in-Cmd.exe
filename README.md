...
### Bengali
এই প্রোগ্রামটি একটি খুব সাধারণ প্রোগ্রাম যা কেবল একটি ব্যাচ ফাইলের সাহায্যে তৈরি হয়েছে যা টার্মিনালে বা Windows cmd-তে চালানো যেতে পারে, অন্য কোনও ভাষার প্রয়োজন ছাড়াই। বর্তমানে আমি বিলম্বের সমস্যার কারণে ফাইলটি আপলোড করছি না।

### Bulgarian
Тази програма е много проста програма, създадена само с помощта на пакетен файл, който може да се стартира в терминала или Windows cmd, без да е необходим друг език. В момента не качвам файла поради проблеми със закъснението.

### Catalan
Aquest programa és un programa molt senzill fet només amb un fitxer batch que es pot executar al terminal o a Windows cmd, sense cap altre llenguatge. Actualment, no estic pujant el fitxer a causa de problemes de retard.

### Croatian
Ovaj program je vrlo jednostavan program napravljen samo s jednim batch datotekom koja se može pokrenuti u terminalu ili Windows cmd, bez ikakvog drugog jezika. Trenutno ne učitavam datoteku zbog problema s kašnjenjem.

### Estonian
See programm on väga lihtne programm, mis on loodud ainult ühe partii failiga, mida saab käitada terminalis või Windows cmd-is, ilma et oleks vaja muud keelt. Praegu ma ei lae faili üles viivituste probleemide tõttu.

### Finnish
Tämä ohjelma on erittäin yksinkertainen ohjelma, joka on tehty vain yhdellä batch-tiedostolla, joka voidaan suorittaa terminaalissa tai Windows cmd:ssä ilman mitään muuta kieltä. Tällä hetkellä en lataa tiedostoa viivästysongelmien vuoksi.

### Gujarati
આ પ્રોગ્રામ ખૂબ જ સરળ પ્રોગ્રામ છે જે ફક્ત એક બેચ ફાઈલનો ઉપયોગ કરીને બનાવવામાં આવ્યું છે જે ટર્મિનલ અથવા Windows cmd માં ચલાવવામાં આવી શકે છે, અન્ય કોઈ ભાષા વિના. હાલમાં હું વિલંબની સમસ્યાના કારણે ફાઈલ અપલોડ કરી રહ્યો નથી.
...
### Indonesian
Program ini adalah program yang sangat sederhana yang dibuat hanya dengan satu file batch yang dapat dijalankan di terminal atau Windows cmd, tanpa bahasa lain. Saat ini, saya tidak mengunggah file tersebut karena masalah keterlambatan.

### Kannada
ಈ ಪ್ರೋಗ್ರಾಮ್ ಅನ್ನು ಟರ್ಮಿನಲ್ ಅಥವಾ ವಿಂಡೋಸ್ cmd ನಲ್ಲಿ ಕಾರ್ಯಗತಗೊಳಿಸಲಾಗುವ ಬ್ಯಾಚ್ ಫೈಲ್ ಅನ್ನು ಬಳಸಿಕೊಂಡು ಮಾತ್ರ ನಿರ್ಮಿಸಲಾಗಿರುವ ಬಹಳ ಸುಲಭವಾದ ಪ್ರೋಗ್ರಾಮ್. ಕಾರ್ಯವಿಧಾನ ವಿಳಂಬದ ಸಮಸ್ಯೆಗಳಿಗೆ ಕಾರಣವಾಗಿರುವ ಕಾರಣ, ನಾನು ಫೈಲ್ ಅನ್ನು ಅಪ್‌ಲೋಡ್ ಮಾಡುತ್ತಿಲ್ಲ.

### Latvian
Šis programmu ir ļoti vienkārša programma, kas izveidota tikai ar vienu partijas failu, ko var palaist terminālā vai Windows cmd, bez jebkādas citas valodas. Šobrīd es neielādēju failu aizkavēšanās problēmu dēļ.

### Lithuanian
Ši programa yra labai paprasta programa, sukurta tik su vienu partijos failu, kurį galima vykdyti terminale arba Windows cmd, be jokių kitų kalbų. Šiuo metu aš neįkeliu failo dėl vėlavimo problemų.

### Malay
Program ini adalah program yang sangat sederhana yang dibuat hanya dengan satu fail batch yang boleh dijalankan di terminal atau Windows cmd, tanpa bahasa lain. Pada masa ini, saya tidak memuat naik fail disebabkan oleh masalah kelewatan.

### Persian
این برنامه یک برنامه بسیار ساده است که فقط با یک فایل دسته‌ای ساخته شده و می‌تواند در ترمینال یا Windows cmd اجرا شود، بدون نیاز به هیچ زبان دیگری. در حال حاضر، من به دلیل مشکلات تأخیر، فایل را بارگذاری نمی‌کنم.

### Serbian
Ovaj program je vrlo jednostavan program koji je napravljen samo sa jednim batch datotekom, koji se može pokrenuti u terminalu ili Windows cmd, bez potrebe za drugim jezikom. Trenutno ne učitavam datoteku zbog problema sa kašnjenjem.

### Slovak
Tento program je veľmi jednoduchý program vytvorený iba pomocou jedného dávkového súboru, ktorý je možné spustiť v termináli alebo Windows cmd, bez potreby iného jazyka. V súčasnosti nenahrávam súbor kvôli problémom so zdržaním.

### Slovenian
Ta program je zelo preprost program, ki je bil ustvarjen le z enim paketnim datotekom, ki ga je mogoče zagnati v terminalu ali Windows cmd, brez potrebe po drugem jeziku. Trenutno ne nalagam datoteke zaradi težav z zamudo.
